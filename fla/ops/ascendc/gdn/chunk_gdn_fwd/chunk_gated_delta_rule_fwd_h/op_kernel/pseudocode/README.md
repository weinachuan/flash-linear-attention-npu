...
GM scratch；gk-only 不读取 gk，同样将 ND `V_new` 写入 GM。随后 Stage2 才用 MTE2 将 GM
ND 搬成 L1 NZ，不能走 UB -> L1 直通路径，否则一次搬运会被拆成多次并阻塞后续指令下发。最终 chunk 且
...
| --- | --- | --- |
| `RunSMinusOneArch35` | `SMinusOneFp32ToBf16Vf`：FP32 initial 转 canonical BF16 H0 | H0 写公开 GM layout-aware，完成后发布 `HGmReady`；S0 再转 L1 NZ |
| `RunStage1Arch35` | `Stage1FullHeadVf`：`U-P`、BF16 `V_new`、g-only 的 `V_new_g/alpha`、首 chunk H0 | 写 `v_new`；有 S2 消费者才写 GM ND right 并发布 `RightGmReady`，S2 再写 L1 NZ |
| `RunStage3Arch35` | `Stage3FullHeadVf`：gate(state)、`+D`、state 原位更新、下一 H 派生 | 非末 chunk 写 H GM layout-aware 并发布 `HGmReady`；末 chunk 按需写 `final_state` |
...
必须等待自己对应的 slot/代际事件。`HGmReady` 只表示 H 的 GM layout-aware 写回完成，`HReady`
只表示 S0 的 GM -> L1 NZ 已完成，不替代 UB state 或 V_new work bank 的 free 事件。
...
个 round 前先等待上一 round 的 kg 消费完成，再按下一 round 的 `requiredKh[]` 重新加载。
...
head.hSlot   -> 当前 H 或 Stage2 右操作数 L1 slot（Stage1 的右操作数在 GM scratch）
...
对 sequence n：
    对 head_round r：
        ...
        如果 r > 0：
            ...
            # 所有等待返回前，禁止发起 H/W 搬运或 Stage2 kg 加载
        ...
        对 chunk c：
            ...
            如果 plan.stage0Required：
                Stage0：搬运 H/W 并计算 P = W @ H
            Stage1：计算 V_new（仅在 S2 需要时计算 V_new_g/alpha）
            如果 plan.stage2Required：
                Stage2：先搬运恰好 Nkg_round 个 kg slot；每个 head 将 GM ND right 转成 L1 NZ，
                        再计算 MMAD D，在 MTE2 后释放 GM scratch、最后一次 MTE1 后释放 kg/right
